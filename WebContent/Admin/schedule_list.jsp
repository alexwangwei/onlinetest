        ...
        <div class="container-fluid">
			...
			<div class="span9" id="content">
				<div class="panel panel-primary">
					<div class="panel-heading">考试排期列表</div>
					<div class="panel-body">
						<form action="adduser" method="post" id="userform">
							<p>
								用户登录名：<input type="text" class="form-control" name="loginId" placeholder="">
							</p>
						...
						<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="categorylist">
							...
							<tbody>
								<s:iterator value="#request.schlist.result" id="res">
									<tr>
										<td><s:property value="#res.name" /></td>
										<td><s:property value="#res.paper.getPaperName()" /></td>
										<td><s:property value="#res.duration/60" /></td>
										<td><s:property value="#res.getFormatedEffectiveDate()" /></td>
										<td><s:property value="#res.getFormatedDisableDate()" /></td>
										<td>
											<s:iterator value="#res.usergroup" id="ug">
												<s:property value="#ug.groupName" />
											</s:iterator>
										</td>
										<td><s:property value="#res.status" /></td>
										<td class="center">
											<s:if test="%{#res.status.toString() == 'ACTIVE'}">
												<a href="./publish?scheduleId=<s:property value="#res.id" />">
												<button class="btn btn-warning">发布 <i class="icon-pencil icon-white"></i></button>
												</a>
											</s:if>
											<a href="./delschedule?scheduleId=<s:property value="#res.id" />">
												<button class="btn btn-danger">删除 <i class="icon-minus icon-white"></i></button>
											</a>
										</td>
									</tr>
								</s:iterator>
						...
						<div class="pagination pagination-centered">
							<ul>
								...
								<s:bean name="org.apache.struts2.util.Counter" id="counter">
									...
									<s:param name="last" value="%{#request.schlist.pageNum}" />
									<s:iterator>
										<li><a href="./listschedule?pageIndex=<s:property />"><s:property /></a></li>
									</s:iterator>
								</s:bean>
